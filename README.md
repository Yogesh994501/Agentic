...
## 3. Interactive Visual Experience & Ergonomics

SentinelFlow is engineered for **command-center clarity** and **live hackathon presentations**:

- 🌐 **Procedural Cyber Background Engine (`60 FPS Canvas`)**:
  - Replaces heavy video files with a zero-dependency, real-time reactive HTML5 Canvas network.
  - **Scroll-Synchronized Threat Energy**: Transitions smoothly from a calm surveillance drift at the top to an accelerated, dense threat constellation as the operator scrolls down into logs and evidence.
- 🎯 **Tactical EMP Click Feedback & HUD Reticle**:
  - Clicking anywhere on the console triggers a localized cyber HUD targeting reticle (`[ ┐ ┘ └ ┌ ]`) with a compact cyan shockwave ring (`72px`), secondary dispersion ring (`110px`), and ambient flare rendered on an independent `z-index: 9999` overlay.
  - Canvas particles in the background network physically deflect radially outward upon click, providing immersive multi-layer visual depth.
- 👓 **Presentation-Grade Typography Hierarchy**:
  - Engineered for instant readability from several feet away during live stage demonstrations:
    - **Primary KPIs**: 26–30px high-contrast metrics (`Active Incidents`, `Confidence %`, `Contained`).
    - **Incident Outcomes**: 20–24px bold status banners (`ATTACK_SUCCEEDED`, `ATTACK_FAILED`).
    - **Content & Evidence**: 13–15px clean monospace and sans-serif body text.
    - **Metadata & Badges**: 11–12px subdued operational tags.
- 💎 **Semi-Opaque Glassmorphic Depth**:
  - UI panels use calibrated 72% opacity (`rgba(10, 17, 30, 0.72)`) with subtle backdrop blur and cyan borders, allowing background light to bleed through while preserving high text contrast.
- 🧭 **Sticky Command Header & Navigation Drawer**:
  - Persistent top navigation with 1-click sandbox reset, real-time agent status pills (`IDLE`, `INVESTIGATING`, `COMPLETED`), and slide-out navigation drawer for jumping directly between simulation, investigation, evidence, and decision traces.

---

## 4. Sandboxed Safe Tools
...
## 5. Five Demonstration Scenarios
...
## 9. Presentation & Documentation Guides
